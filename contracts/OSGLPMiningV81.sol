 ...
 *
 *  forfeitAndWithdraw() is the failure-proof exit, not an early one. It
 *  makes no call to RewardPool -- so no failure anywhere else in the
 *  system can hold someone's LP -- and it is not whenNotPaused. But it
 *  respects the term exactly as withdraw() does: termLifted, or the 365
 *  days are up. What it gives up is the reward accrued and not yet taken.
 *
 *  v7 shipped with this paragraph claiming the exit worked "at any
 *  moment, from the first block onwards". The code never did that. The
 *  wording is corrected here because an operator reading it could plan
 *  a migration around an exit that does not exist.
 *
...
contract OSGLPMining is Ownable, ReentrancyGuard, Pausable {
    ...
    /// How long each position must be held before the LP comes back.
    /// Both exits enforce it -- withdraw() and forfeitAndWithdraw()
    /// alike. Only liftTerm() releases it early.
    ...
    ///
    /// V8 note: the live pairing at redeploy is
    ///     LPMining 5,932 + TermStakingV2 4,068 = 10,000 exactly,
    /// so there is no slack left. Lower one before raising the other, or
    /// the sum crosses 10,000 for the blocks in between.
    ...
    /// start getting capped and deferred and neither rate holds.
    /// V8 note: V7 shipped with maxShareBps = 5_000 and it had to be
    /// raised on-chain before the intended 5_932 share could be set.
    /// Starting at the value actually in use removes that wiring step;
    /// ABSOLUTE_MAX_SHARE_BPS (7_000) is still the hard ceiling.
    uint256 public miningShareBps = 5_932;
    uint256 public maxShareBps    = 5_932;

    /// Open positions per wallet, maintained incrementally.
    ///
    /// V7 counted open positions by walking the whole history on every
    /// deposit. MAX_POSITIONS caps how many a wallet holds OPEN, not how
    /// many it has ever held, so a wallet that cycles positions grows an
    /// array that every later deposit has to scan. This counter makes the
    /// check O(1). It is incremented in deposit() and decremented on both
    /// close paths -- withdraw() and forfeitAndWithdraw(). Those three
    /// sites are the whole invariant; openPositions() still recomputes
    /// from the array so the two can be compared off-chain.
    ...
    ///
    /// MAINTENANCE: this is a snapshot, not a rule. The true figure is
    ///     2 * osgReserve / lpTotalSupply
    /// on the QuickSwap pair, and it drifts as the pool grows. V7 ran
    /// with 2.0 while the true value was 0.645 -- every position was
    /// priced 3.1x high and the error was invisible from inside the
    /// contract. Re-read the pair and compare every 2-3 weeks.
    ...
    function deposit(uint256 lpAmount) external nonReentrant whenNotPaused {
        ...
        require(openPositionCount[msg.sender] < MAX_POSITIONS, "position limit reached");
        ...
        totalLp         += received;
        openPositionCount[msg.sender] += 1;
    ...
    /// The escape hatch. Not whenNotPaused, and it needs no SUCCESSFUL
    /// RewardPool call -- but it does NOT ignore the term. Like
    /// withdraw(), it requires termLifted or the 365 days to be up. The
    /// difference is what it survives, not when it is allowed.
    ///
    ...
    /// Recomputed from the array. O(n) -- for off-chain comparison
    /// against openPositionCount, not for use on a write path.
    ...
    /// exits: once the term allows it, forfeitAndWithdraw() works
