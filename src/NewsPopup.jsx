<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
<title>OSG P2P Popup — Digital Fresh</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,700;12..96,800;12..96,900&family=Hanken+Grotesk:wght@400;500;600;700;800&display=swap');
  *{margin:0;padding:0;box-sizing:border-box}
  body{
    font-family:'Hanken Grotesk',sans-serif;
    background:#030305;
    min-height:100vh;
    display:flex;align-items:center;justify-content:center;
    padding:24px;
  }

  /* holographic rotating border */
  .frame{
    width:100%;max-width:380px;
    border-radius:26px;
    padding:2px;
    position:relative;
    background:conic-gradient(from var(--a,0deg),
      #F7D27A 0deg, #38BDF8 90deg, #A78BFA 180deg, #46D08A 270deg, #F7D27A 360deg);
    animation:spin 7s linear infinite;
    box-shadow:0 30px 90px -14px rgba(56,189,248,.25), 0 30px 80px rgba(0,0,0,.7);
  }
  @property --a{ syntax:'<angle>'; inherits:false; initial-value:0deg; }
  @keyframes spin{to{--a:360deg}}

  .phone{
    border-radius:24px;
    overflow:hidden;
    position:relative;
    background:#07070A;
  }

  /* mesh gradient blobs */
  .mesh{position:absolute;inset:0;pointer-events:none;overflow:hidden}
  .blob{position:absolute;border-radius:50%;filter:blur(46px);opacity:.55}
  .b1{width:220px;height:220px;top:-90px;left:-60px;background:radial-gradient(circle,#F7D27A,transparent 70%)}
  .b2{width:200px;height:200px;top:-70px;right:-80px;background:radial-gradient(circle,#38BDF8,transparent 70%);opacity:.4}
  .b3{width:180px;height:180px;bottom:-90px;left:40%;background:radial-gradient(circle,#A78BFA,transparent 70%);opacity:.3}

  .noise{
    position:absolute;inset:0;pointer-events:none;
    background-image:radial-gradient(rgba(255,255,255,.035) 1px, transparent 1px);
    background-size:22px 22px;
  }

  .modal{position:relative;z-index:2;padding:20px 22px 18px}

  .close{
    position:absolute;top:14px;right:14px;
    width:30px;height:30px;border-radius:50%;
    background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.14);
    backdrop-filter:blur(6px);
    display:flex;align-items:center;justify-content:center;
    color:#E4E4EA;font-size:14px;
  }

  .brand{display:flex;align-items:center;gap:10px;margin-top:0}
  .logo-wrap{position:relative;width:46px;height:46px;flex:none}
  .logo-halo{
    position:absolute;inset:-10px;border-radius:50%;
    background:conic-gradient(from 0deg,#F7D27A,#38BDF8,#A78BFA,#F7D27A);
    filter:blur(8px);opacity:.65;
    animation:spin 7s linear infinite;
  }
  .logo-img{width:46px;height:46px;object-fit:contain;position:relative;border-radius:50%;
    box-shadow:0 0 0 3px #07070A;}
  .name{
    font-family:'Bricolage Grotesque';font-weight:900;font-size:21px;letter-spacing:.5px;line-height:1;
    background:linear-gradient(120deg,#FCEAB0,#F7D27A 40%,#38BDF8 75%,#A78BFA 100%);
    -webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;
  }
  .sub{font-size:9.5px;letter-spacing:2px;color:#5CE0B0;font-weight:700;margin-top:3px}

  .badge{
    display:inline-flex;align-items:center;gap:7px;
    margin-top:16px;
    font-size:11.5px;font-weight:800;letter-spacing:.5px;color:#050507;
    background:linear-gradient(120deg,#F7D27A,#5CE0F0 90%);
    border-radius:99px;padding:7px 14px 7px 10px;
    box-shadow:0 6px 20px -4px rgba(56,189,248,.5);
  }
  .badge .pulse{
    width:7px;height:7px;border-radius:50%;background:#050507;
    animation:blink 1.3s ease-in-out infinite;
  }
  @keyframes blink{0%,100%{opacity:1}50%{opacity:.25}}

  h1.headline{
    font-family:'Bricolage Grotesque';font-weight:900;font-size:23px;line-height:1.16;
    color:#F7F7FA;margin-top:10px;letter-spacing:-.4px;
  }
  h1.headline .glow{
    background:linear-gradient(100deg,#F7D27A,#5CE0F0 60%,#A78BFA);
    -webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;
  }
  .rule{
    width:36px;height:3px;border-radius:99px;margin-top:9px;
    background:linear-gradient(90deg,#F7D27A,#38BDF8);
  }
  p.desc{font-size:12.5px;color:#9E9EAB;line-height:1.5;margin-top:9px}

  .features{margin-top:14px;display:flex;flex-direction:column;gap:8px}
  .feat{
    display:flex;align-items:center;gap:11px;
    background:rgba(255,255,255,.035);
    border-radius:12px;padding:9px 12px;
    border:1px solid transparent;
    position:relative;
  }
  .feat::before{
    content:"";position:absolute;inset:0;border-radius:14px;padding:1px;
    background:var(--edge);
    -webkit-mask:linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0);
    -webkit-mask-composite:xor;mask-composite:exclude;
  }
  .feat.f1{--edge:linear-gradient(120deg,#F7D27A,#8a6318)}
  .feat.f2{--edge:linear-gradient(120deg,#5CE0F0,#38BDF8)}
  .feat .ic{
    flex:none;width:28px;height:28px;border-radius:9px;
    display:flex;align-items:center;justify-content:center;font-size:13px;
  }
  .feat.f1 .ic{background:rgba(233,185,73,.16);color:#F7D27A}
  .feat.f2 .ic{background:rgba(56,189,248,.16);color:#5CE0F0}
  .feat .tt{font-size:12.5px;font-weight:800;color:#F0F0F4}

  .cta{
    margin-top:16px;width:100%;border:none;cursor:pointer;
    font-family:'Hanken Grotesk';font-weight:800;font-size:14.5px;color:#050507;
    background:linear-gradient(100deg,#F7D27A 0%,#5CE0F0 55%,#A78BFA 100%);
    background-size:200% 100%;
    border-radius:13px;padding:13px;
    display:flex;align-items:center;justify-content:center;gap:8px;
    box-shadow:0 14px 34px -8px rgba(56,189,248,.45), inset 0 1px 0 rgba(255,255,255,.5);
    animation:shift 3.5s ease-in-out infinite;
  }
  @keyframes shift{0%,100%{background-position:0% 0}50%{background-position:100% 0}}

  .disclaimer{
    display:flex;align-items:center;justify-content:center;gap:6px;
    text-align:center;font-size:9.5px;color:#6B6B78;margin-top:10px;
  }
</style>
</head>
<body>
  <div class="frame">
    <div class="phone">
      <div class="mesh"><div class="blob b1"></div><div class="blob b2"></div><div class="blob b3"></div></div>
      <div class="noise"></div>

      <div class="modal">
        <div class="close">✕</div>

        <div class="brand">
          <div class="logo-wrap">
            <div class="logo-halo"></div>
            <img class="logo-img" src="data:image/jpeg;base64," alt="OSG"/>
          </div>
          <div>
            <div class="name">OSG</div>
            <div class="sub">ONEX SMART GOLD</div>
          </div>
        </div>

        <span class="badge"><span class="pulse"></span>JUST LAUNCHED</span>

        <h1 class="headline">P2P Exchange <span class="glow">v2</span> — Auto-Match is here!</h1>
        <div class="rule"></div>
        <p class="desc">Place an order and it fills instantly at the best price. No more waiting to get matched.</p>

        <div class="features">
          <div class="feat f1"><div class="ic">⚡</div><div class="tt">Instant auto-matching</div></div>
          <div class="feat f2"><div class="ic">✅</div><div class="tt">Same low 0.5% fee, on-chain</div></div>
        </div>

        <button class="cta">🚀 Try P2P Exchange</button>
        <div class="disclaimer">🛡️ Trading involves risk. Always DYOR.</div>
      </div>
    </div>
  </div>
</body>
</html>
